...
class Domain_For_Sale_Posttype
{
    ...
    public function __construct($args = array())
    {
        ...
        $this->post_type_name = self::domain_for_sale_postTypeName_prepare($args['post_type']);
        ...
        if (!post_type_exists($this->post_type_name)) {
            ...
            add_action('init', array($this, 'domain_for_sale_register_post_type'));
        }
    }
    // Post type register
    public function domain_for_sale_register_post_type()
    {
        ...
        $defaultargs = array(
            ...
            'show_in_menu'       => true,
            'query_var'          => true,
            'rewrite'            => array('slug' => $this->domain_for_sale_slug_prepare($this->reg_type_name)),
    ...
    // register taxonomy 
    public function domain_for_sale_register_taxonomy($args = array())
    {
        ...
        if (!empty($args['taxname'])) {
            ...
            $taxName      = self::domain_for_sale_postTypeName_prepare($args['taxname']);
        }
        ...
        $defaultArgs = array(
            ...
            'query_var'         => true,
            'rewrite'           => array('slug' => $this->domain_for_sale_slug_prepare($taxName)),
        );
    ...
    private static function domain_for_sale_postTypeName_prepare($name)
    {
    ...
    // Label Plural name
    private static function domain_for_sale_pluralName($name)
    {

        $name = self::domain_for_sale_namePrepare($name);
    ...
    // label name prepare
    private static function domain_for_sale_namePrepare($name)
    {
    ...
    // slug prepare
    private function domain_for_sale_slug_prepare($name)
    {
