 ...
 *
 */
if ( ! class_exists( 'DOMAIN_FOR_SALE_Field_wp_editor' ) ) {
  class DOMAIN_FOR_SALE_Field_wp_editor extends DOMAIN_FOR_SALE_Fields {
    ...
    public function enqueue() {
      ...
      if ( DOMAIN_FOR_SALE_wp_editor_api() && function_exists( 'wp_enqueue_editor' ) ) {
    ...
    public function setup_wp_editor_media_buttons() {
      ...
      echo 'var DOMAIN_FOR_SALE_media_buttons = '. json_encode( $media_buttons ) .';';
      echo '</script>';
    ...
    public function setup_wp_editor_settings() {
      ...
      if ( DOMAIN_FOR_SALE_wp_editor_api() && class_exists( '_WP_Editors') ) {

        $defaults = apply_filters( 'DOMAIN_FOR_SALE_wp_editor', array(
          'tinymce' => array(
            'wp_skip_init' => true
          ),
        ) );

        $setup = _WP_Editors::parse_settings( 'DOMAIN_FOR_SALE_wp_editor', $defaults );

        _WP_Editors::editor_settings( 'DOMAIN_FOR_SALE_wp_editor', $setup );
