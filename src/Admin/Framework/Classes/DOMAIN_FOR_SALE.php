 ...
 */
 ...
 namespace ThemeAtelier\DomainForSale\Admin\Framework\Classes;

if ( ! class_exists( 'DOMAIN_FOR_SALE' ) ) {
  class DOMAIN_FOR_SALE {
    ...
    public function __construct() {
      ...
      // Init action
      do_action( 'DOMAIN_FOR_SALE_init' );

      add_action( 'after_setup_theme', array( 'ThemeAtelier\DomainForSale\Admin\Framework\Classes\DOMAIN_FOR_SALE', 'setup' ) );
      add_action( 'init', array( 'ThemeAtelier\DomainForSale\Admin\Framework\Classes\DOMAIN_FOR_SALE', 'setup' ) );
      add_action( 'switch_theme', array( 'ThemeAtelier\DomainForSale\Admin\Framework\Classes\DOMAIN_FOR_SALE', 'setup' ) );
      add_action( 'admin_enqueue_scripts', array( 'ThemeAtelier\DomainForSale\Admin\Framework\Classes\DOMAIN_FOR_SALE', 'add_admin_enqueue_scripts' ) );
      add_action( 'wp_enqueue_scripts', array( 'ThemeAtelier\DomainForSale\Admin\Framework\Classes\DOMAIN_FOR_SALE', 'add_typography_enqueue_styles' ), 80 );
      add_action( 'wp_head', array( 'ThemeAtelier\DomainForSale\Admin\Framework\Classes\DOMAIN_FOR_SALE', 'add_custom_css' ), 80 );
      add_filter( 'admin_body_class', array( 'ThemeAtelier\DomainForSale\Admin\Framework\Classes\DOMAIN_FOR_SALE', 'add_admin_body_class' ) );
    ...
    public static function setup() {
      ...
      $params = array();
      if ( class_exists( 'DOMAIN_FOR_SALE_Options' ) && ! empty( self::$args['admin_options'] ) ) {
        foreach ( self::$args['admin_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            \DOMAIN_FOR_SALE_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'DOMAIN_FOR_SALE_Metabox' ) && ! empty( self::$args['metabox_options'] ) ) {
    ...
    public static function include_plugin_file( $file, $load = true ) {
      ...
      $path     = '';
      $file     = ltrim( $file, '/' );
      $override = apply_filters( 'DOMAIN_FOR_SALE_override', 'domain-for-sale-override' );
    ...
    public static function includes() {
      ...
      self::include_plugin_file( 'Classes/DOMAIN_FOR_SALE_Options.php' );
      self::include_plugin_file( 'Classes/DOMAIN_FOR_SALE_Metabox.php'   );

      // Include all framework fields
      $fields = apply_filters( 'DOMAIN_FOR_SALE_fields', array(
        'accordion',
        'background',
        'backup',
        'border',
      ...
      if ( ! empty( $fields ) ) {
        foreach ( $fields as $field ) {
          if ( ! class_exists( 'DOMAIN_FOR_SALE_Field_'. $field ) && class_exists( 'DOMAIN_FOR_SALE_Fields' ) ) {
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      if ( ! self::$enqueue ) {
        ...
        if ( $wpscreen->id === 'tools_page_domain-for-sale-welcome' ) {
          self::$enqueue = true;
        }

      }

      if ( ! apply_filters( 'DOMAIN_FOR_SALE_enqueue_assets', self::$enqueue ) ) {
        return;
      }
      ...
      // Main style
      wp_enqueue_style( 'domain-for-sale', self::include_plugin_url( 'assets/css/style'. $min .'.css' ), array(), self::$version, 'all' );
      ...
      if ( ! empty( self::$fields ) ) {
        foreach ( self::$fields as $field ) {
          if ( ! empty( $field['type'] ) ) {
            $classname = 'DOMAIN_FOR_SALE_Field_' . $field['type'];
      ...
      do_action( 'DOMAIN_FOR_SALE_enqueue' );
    ...
    public static function add_typography_enqueue_styles() {
      ...
      if ( ! empty( self::$webfonts ) ) {
        ...
        if ( ! empty( self::$webfonts['enqueue'] ) ) {
          ...
          wp_enqueue_style( 'domain-for-sale-google-web-fonts', esc_url( add_query_arg( $query, '//fonts.googleapis.com/css' ) ), array(), null );
        ...
        if ( ! empty( self::$webfonts['async'] ) ) {
          ...
          wp_enqueue_script( 'domain-for-sale-google-web-fonts', esc_url( '//ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js' ), array(), null );

          wp_localize_script( 'domain-for-sale-google-web-fonts', 'WebFontConfig', array( 'google' => array( 'families' => $fonts ) ) );
    ...
    public static function add_admin_body_class( $classes ) {
      ...
      if ( apply_filters( 'DOMAIN_FOR_SALE_fa4', false ) ) {
        $classes .= 'domain-for-sale-fa5-shims';
      }
    ...
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
      ...
      $is_pseudo  = ( ! empty( $field['pseudo'] ) ) ? ' domain-for-sale-pseudo-field' : '';
      ...
      echo '<div class="domain-for-sale-field domain-for-sale-field-'. $field_type . $is_pseudo . $class . $visible .'"'. $depend .'>';

      if ( ! empty( $field_type ) ) {

        if ( ! empty( $field['title'] ) ) {
          echo '<div class="domain-for-sale-title">';
          echo '<h4>'. $field['title'] .'</h4>';
          echo ( ! empty( $field['subtitle'] ) ) ? '<div class="domain-for-sale-subtitle-text">'. $field['subtitle'] .'</div>' : '';
          echo '</div>';
        }

        echo ( ! empty( $field['title'] ) ) ? '<div class="domain-for-sale-fieldset">' : '';

        $value = ( ! isset( $value ) && isset( $field['default'] ) ) ? $field['default'] : $value;
        $value = ( isset( $field['value'] ) ) ? $field['value'] : $value;

        $classname = 'DOMAIN_FOR_SALE_Field_'. $field_type;
